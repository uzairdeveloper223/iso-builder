/**
 * This code is responsible for creating a Debian root filesystem for the
 * payload system (gets installed to disk by the installer).
 */
...
int create_payload_rootfs(const char *path)
{
    ...
    // Install all packages needed by the payload system.
